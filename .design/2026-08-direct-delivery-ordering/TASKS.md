...
- [x] **Edge action 13개 명세**: `storefront`, `create_session`,
  ...
  `staff_proof_url`, `cleanup_expired_pii`
...
| 고객/직원 자유 채팅 | 언어 선택과 무관하게 작성자 원문을 그대로 표시 |
...
- [x] **viewer-locale 계약 문서화**: `request.locale`은 고객이 요청을
  ...
  free text는 작성자 원문을 그대로 보존·표시한다. _생성:
...
- [x] **system message·주소·채팅 언어 경계 적용**: DB에는 고정 system code를
  ...
  응답 그대로 보존한다. 고객/cashier chat과 상세주소·note는 작성된 원문을
  그대로 표시하고 외부 번역 서비스를 호출하지 않는다. _수정: direct customer/staff message
...
- [x] **교차 언어 E2E matrix 추가**: 고객 locale `ko/vi/en` × 캐셔 locale
  ...
  영향이 없어야 한다. chat free text는 모든 viewer locale에서 작성자 원문을
  그대로 표시한다. _확장: direct
...
- 고객도 현재 선택한 `ko/vi/en`으로 UI를 보고, 채팅은 모든 화면에서
  작성자 원문을 그대로 표시한다. 주소와 note도 원문이다.
