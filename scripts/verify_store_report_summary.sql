DO $$
DECLARE v_oid oid := to_regprocedure('public.get_store_report_summary(uuid,date,date)');
BEGIN
  IF v_oid IS NULL OR NOT EXISTS (SELECT 1 FROM pg_proc WHERE oid = v_oid
    ...
    RAISE EXCEPTION 'STORE_REPORT_FUNCTION_INVALID';
  END IF;
  IF has_function_privilege('anon', v_oid, 'EXECUTE')
    ...
    RAISE EXCEPTION 'STORE_REPORT_PRIVILEGES_INVALID';
  END IF;
END;
$$;
SELECT 'store report summary verification passed' AS result;
