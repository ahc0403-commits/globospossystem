DO $$
DECLARE v_oid oid := to_regprocedure('public.get_payroll_hourly_rules(uuid,uuid[])');
BEGIN
  IF v_oid IS NULL OR NOT EXISTS (SELECT 1 FROM pg_proc WHERE oid = v_oid
    ...
    RAISE EXCEPTION 'PAYROLL_RULES_FUNCTION_INVALID';
  END IF;
  IF has_function_privilege('anon', v_oid, 'EXECUTE')
    ...
    RAISE EXCEPTION 'PAYROLL_RULES_PRIVILEGES_INVALID';
  END IF;
END $$;
