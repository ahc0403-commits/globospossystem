...
BEGIN
  IF v_oid IS NULL OR NOT EXISTS (SELECT 1 FROM pg_proc WHERE oid = v_oid
    ...
    RAISE EXCEPTION 'PROMOTION_SYNC_FUNCTION_INVALID';
  END IF;
  SELECT pg_get_functiondef(v_oid) INTO v_definition;
  IF position('v_existing_lines IS NOT DISTINCT FROM v_desired_lines' IN v_definition) = 0
     OR position('PROMOTION_ALLOCATION_MISMATCH' IN v_definition) = 0 THEN
    RAISE EXCEPTION 'PROMOTION_SYNC_IDEMPOTENCE_MISSING';
  END IF;
  IF has_function_privilege('anon', v_oid, 'EXECUTE')
     OR has_function_privilege('authenticated', v_oid, 'EXECUTE')
     OR NOT has_function_privilege('service_role', v_oid, 'EXECUTE') THEN
    RAISE EXCEPTION 'PROMOTION_SYNC_PRIVILEGES_INVALID';
  END IF;
END $$;
SELECT 'idempotent promotion sync verification passed' AS result;
