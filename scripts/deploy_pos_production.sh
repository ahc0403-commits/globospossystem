...
apply_migration() {
  ...
  elif [[ "$migration_name" == "20260723060000_photo_ops_latest_sales_rpc.sql" ]]; then
    log "Photo Ops latest-sales RPC migration preflight"
    run_linked_psql_file \
  ...
  elif [[ "$migration_name" == "20260723081529_photo_objet_simple_inventory_management.sql" ]]; then
    log "Photo Objet simple inventory management migration preflight"
    run_linked_psql_file \
  ...
  elif [[ "$migration_name" == "20260727013210_inventory_ingredient_excel_supplier_banking.sql" ]]; then
    log "Inventory ingredient Excel and supplier banking migration preflight"
    run_linked_psql_file \
  ...
  elif [[ "$migration_name" == "20260804010000_scheduled_closing_and_promotions.sql" ]]; then
    log "Scheduled closing and promotions migration preflight"
    run_linked_psql_file \
  ...
  if [[ "$migration_name" == "20260711090000_legal_entity_brand_store_hierarchy.sql" ]]; then
  ...
  elif [[ "$migration_name" == "20260723020000_inventory_supplier_bank_account.sql" ]]; then
    log "Inventory supplier bank-account migration verification"
    run_linked_psql_file \
    ...
    log "Photo Ops latest-sales RPC migration verification"
    run_linked_psql_file \
    ...
    log "Photo Objet simple inventory management migration verification"
    run_linked_psql_file \
  ...
  elif [[ "$migration_name" == "20260724025456_attendance_logs_with_names.sql" ]]; then
    log "Attendance names migration verification"
    run_linked_psql_file \
    ...
    log "Inventory ingredient Excel and supplier banking migration verification"
    run_linked_psql_file \
    ...
    log "Scheduled closing and promotions migration verification"
    run_linked_psql_file \
