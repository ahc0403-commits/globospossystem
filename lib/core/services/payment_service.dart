  ...
  Future<Map<String, dynamic>?> fetchPaymentDetail(
    ...
    if (orderId != null && orderId.isNotEmpty) {
      ...
      final orderQuery = supabase
          .from('orders')
          .select(
            'id, restaurant_id, table_id, status, created_at, updated_at, tables(table_number), order_items(id, created_at, status, label, unit_price, quantity, is_service_item, service_reason, vat_amount, paying_amount_inc_tax, menu_items(name, name_ko, name_vi, name_en))',
          )
          .eq('id', orderId);
