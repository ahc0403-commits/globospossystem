...
|---|---|---|---|---|---|---|
| Waiter `/waiter`<br>`lib/features/waiter/waiter_screen.dart` | Select or continue service for a table. | Filter/scan tables, select table, enter guest count, open active order, start order, create staff-meal order with manager PIN. | Table history, full order history, transfer detail, audit trace. | Full order editing can appear immediately after table selection through `OrderWorkspace`. | Table grid, table state, guest prompt, selected-table summary, start/continue order, staff-meal entry action. | Payment execution, kitchen item status control, manager cancellation approval, reports. |
...
| Kitchen Queue<br>`lib/features/kitchen/kitchen_screen.dart` | Decide which kitchen ticket needs work next. | Scan lanes, open ticket, identify delay, refresh queue, group by status, identify staff-meal tickets by badge. | Full modifiers, item history, served history, long notes, audit trace. | Item-level execution is currently embedded inside ticket cards. | New/Cooking/Ready/Delayed lanes, ticket summary, elapsed time, priority signal, staff-meal badge. | Full item execution panel, stock-out request handling, manager reports. |
