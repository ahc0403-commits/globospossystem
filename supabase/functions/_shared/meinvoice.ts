...
// Official MISA docs:
// https://doc.meinvoice.vn/itg/Doc/GetToken.html
// https://doc.meinvoice.vn/itg/Doc/publish/SignType5.html
// https://doc.meinvoice.vn/itg/Doc/GetTemplate.html
// https://doc.meinvoice.vn/itg/Doc/GetStatusInvoice.html
// https://doc.meinvoice.vn/itg/Doc/DowloadInvoice.html
// https://doc.meinvoice.vn/itg/Doc/Info/InvoiceData.html
...
function secretForTaxCode(baseName: string, taxCode: string): string | null {
  const taxSpecific = Deno.env.get(
    `${baseName}_${envSuffixForTaxCode(taxCode)}`,
  );
  return asString(taxSpecific) ?? asString(Deno.env.get(baseName));
}
...
export function assertDispatchReady(seller: SellerConfig) {
  ...
  if (!seller.appId) throw new Error("MEINVOICE_APP_ID_NOT_CONFIGURED");
...
function lineItemsForJob(job: JsonRecord): JsonRecord[] {
  ...
  return source.map((item, index) => {
    ...
    const amountWithoutVat = roundMoney(
      asNumber(item.total_amount_ex_tax, unitPrice * quantity),
    );
    const vatAmount = roundMoney(asNumber(item.vat_amount, 0));
...
function taxRateInfo(details: JsonRecord[]): JsonRecord[] {
  ...
  return [...grouped.entries()].map(([VATRateName, totals]) =>
    compact({
      VATRateName,
      AmountWithoutVATOC: roundMoney(totals.amount),
      AmountWithoutVAT: roundMoney(totals.amount),
      VATAmountOC: roundMoney(totals.vat),
...
export function buildCashRegisterInvoicePayload(
  ...
  const totalPayment = roundMoney(totalWithoutVat + totalVat);
