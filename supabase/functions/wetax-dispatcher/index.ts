/**
 * wetax-dispatcher — v4 (WT03 fix 2026-04-13)
 * Vendor confirmed: correct endpoint is /pos/invoices (WT03), not /pos/sendOrderInfo
 * sid is returned immediately from WT03 response — AP1 eliminated
 */
...
async function processSendOrder(supabase: any, job: any, token: string, pollingEnabled: boolean) {
  ...
  // WT03 /pos/invoices — correct endpoint confirmed by vendor 2026-04-13
  ...
  // WT03 returns data:[{ref_id, sid, status:{code,message}}]
  // sid returned immediately (AP1 eliminated)
...
async function processRequestEinvoice(supabase: any, job: any, token: string, config: Record<string,string>) {
  ...
  await logEvent(supabase,job.id,"request_einvoice_attempt",
    `WT05 /invoices-issue ${result.ok?"success":"failed"} HTTP ${result.status}`,
