...
BEGIN
  ...
  BEGIN
    PERFORM public.cancel_order_item(item, store);
  ...
  BEGIN
    PERFORM public.cancel_order_item(item, gen_random_uuid());
    RAISE EXCEPTION 'Cross-store cancellation unexpectedly succeeded';
  EXCEPTION WHEN OTHERS THEN
    IF SQLERRM <> 'ORDER_MUTATION_FORBIDDEN' THEN RAISE; END IF;
  END;

  SELECT * INTO result FROM public.cancel_order_item(item, store);
  ...
  END IF;
  IF (SELECT count(*) FROM public.order_cancellation_ledger WHERE order_item_id = item) <> 1 THEN
    RAISE EXCEPTION 'Cancellation ledger missing';
  END IF;
  PERFORM public.restore_cancelled_order_item(item, store);
  ...
  END;
  RAISE NOTICE 'PASS: checked/unchecked menu cancellation, ledger, KDS sync, undo, roles, scope, payment guard';
END;
$$;
ROLLBACK;
