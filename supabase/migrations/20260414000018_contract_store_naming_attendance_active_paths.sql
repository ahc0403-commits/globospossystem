-- ============================================================
-- Contract phase: rename active attendance RPC inputs to store naming
-- 2026-04-14
-- Scope:
...
-- Notes:
-- - physical schema and storage policies still use restaurant_id-era columns
-- - attendance-photos path keeps first segment = active store UUID during coexistence
-- ============================================================

DROP FUNCTION IF EXISTS public.get_attendance_staff_directory(uuid);
DROP FUNCTION IF EXISTS public.get_attendance_log_view(uuid, timestamptz, timestamptz, uuid);
DROP FUNCTION IF EXISTS public.record_attendance_event(uuid, uuid, text, text, text);
