-- Bundle E-1: office internal system external email integration.
-- Scope:
-- 1. Add email gating booleans to system.system_settings.
-- 2. Expand system.get_system_settings() / system.update_system_settings().
-- 3. Replace the audit-log email enqueue handler for deterministic Office events.
--
-- Grounded recipient mappings from repo truth:
-- - purchase_approved / purchase_rejected:
--   accounting.purchase_requests.requested_by -> public.office_user_profiles(auth_id)
--   added in migration 254_fix_purchases_canonical_path.sql
-- - payroll_confirmed / payroll_rejected:
--   mirror existing in-app notify_payroll_confirmed() audience from migrations
--   240/260/270: all active office_user_profiles with account_level in
--   ('super_admin', 'brand_admin')
--
-- Grounded blocker:
-- - expense_approved / expense_returned:
--   accounting.expenses.submitted_by is TEXT in current repo truth (migration 050)
--   with no deterministic auth/profile linkage. Do not enqueue until a separate
--   schema/write-path change provides immutable submitter identity.
  ...
  if new.action = 'account_created'
    ...
    if found then
      ...
      on conflict do nothing;
        ...
        and new.entity_id is not null then
    -- Grounded blocker: accounting.expenses.submitted_by is TEXT only in repo
    -- truth, so there is no deterministic recipient_email to snapshot.
    return new;
  end if;
