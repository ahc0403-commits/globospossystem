-- Read-only, role-scoped ledger for a selected HCM business date.
...
COMMENT ON FUNCTION public.get_receipt_ledger(
  date, uuid, text, text, integer, integer
) IS 'Role-scoped, read-only receipt ledger for a selected HCM business date. POS rows aggregate split payments by order.';
...
BEGIN
  ...
  IF v_security_definer IS DISTINCT FROM true
    ...
    RAISE EXCEPTION 'RECEIPT_LEDGER_VERIFICATION_FAILED';
  END IF;
END;
$$;
