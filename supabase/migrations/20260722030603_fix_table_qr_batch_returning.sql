-- Qualify INSERT ... RETURNING columns so the RETURNS TABLE output variable
-- `table_id` cannot shadow the table_qr_tokens column in PL/pgSQL.
...
BEGIN
  ...
  WITH candidates AS (
    ...
    INSERT INTO public.table_qr_tokens AS created_token (
      restaurant_id,
      table_id,
      token,
      created_by
    ...
    RETURNING
      created_token.id,
      created_token.restaurant_id,
      created_token.table_id
  )
