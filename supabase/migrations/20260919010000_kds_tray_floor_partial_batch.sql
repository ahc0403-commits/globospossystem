-- Allow tray staff to dispatch an explicit subset of the currently ready food.
-- Selected quantities remain atomic and stale-safe; unselected or newly ready
-- quantities stay pending for a later floor-delivery batch.
...
BEGIN
  ...
  -- Lock only the selected lines. Kitchen completions that finish before this
  -- barrier may increase the available quantity and remain pending; work that
  -- reaches the same line afterwards waits until this batch is committed.
  BEGIN
    PERFORM 1
  ...
  -- A partial selection is valid when every selected line still has at least
  -- the requested quantity. Other lines and excess quantity are deliberately
  -- excluded so they remain available for the next batch.
  IF EXISTS (
    SELECT 1
    FROM jsonb_to_recordset(v_client_snapshot) AS allocation(
      item_id uuid, queue_id uuid, source_kind text, quantity integer
    )
    WHERE NOT EXISTS (
      SELECT 1
      ...
      WHERE current_line.item_id = allocation.item_id
        AND current_line.queue_id = allocation.queue_id
        AND current_line.source_kind = allocation.source_kind
        AND current_line.quantity >= allocation.quantity
    )
  ) THEN
    RAISE EXCEPTION 'KDS_TRAY_FLOOR_BATCH_STALE';
  END IF;
...
BEGIN
  ...
  IF position(
     ...
     OR position('FOR UPDATE OF item, queue NOWAIT' IN v_definition) = 0 THEN
    RAISE EXCEPTION 'KDS_TRAY_FLOOR_PARTIAL_BATCH_VERIFY_FAILED';
  END IF;
END;
$verify$;
