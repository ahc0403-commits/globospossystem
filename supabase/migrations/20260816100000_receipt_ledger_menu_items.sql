-- Include the ordered menu lines in each POS receipt ledger entry.
-- External sales do not have POS order_items and therefore return an empty list.
...
BEGIN
  ...
  pos_order_items AS (
    ...
    JOIN pos_payment_groups payment_group
      ON payment_group.order_id = item.order_id
     AND payment_group.store_id = item.restaurant_id
  ...
  all_receipts AS (
    ...
    LEFT JOIN pos_order_items order_items
      ON order_items.order_id = payment_group.order_id
...
COMMENT ON FUNCTION public.get_receipt_ledger(
  date, uuid, text, text, integer, integer
) IS 'Role-scoped receipt ledger for an HCM business date, including POS order item snapshots.';
...
BEGIN
  ...
  IF v_security_definer IS DISTINCT FROM true
     ...
     OR position('pos_order_items AS' IN v_definition) = 0
     OR position('item.status <> ''cancelled''' IN v_definition) = 0 THEN
    RAISE EXCEPTION 'RECEIPT_LEDGER_MENU_ITEMS_VERIFICATION_FAILED';
  END IF;
END;
$$;
