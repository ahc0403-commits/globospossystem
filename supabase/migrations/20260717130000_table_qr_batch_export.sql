-- Stable table QR batch export support.
--
-- This migration adds no tables, columns, storage objects, or server files.
-- Existing active table QR tokens are reused. Token invalidation remains the
-- responsibility of the explicit admin_generate_table_qr(uuid) rotation RPC.
...
BEGIN
  ...
  -- Lock canonical table rows in the same deterministic order used by the
  -- exported artifacts. The partial unique index remains the final concurrent
  -- writer guard if two sessions request the same table simultaneously.
  PERFORM 1
  ...
  WITH candidates AS (
    ...
    ON CONFLICT (table_id) WHERE is_active DO NOTHING
