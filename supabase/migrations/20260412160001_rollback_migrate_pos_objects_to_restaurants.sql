-- ============================================================================
-- Phase 2 Step 2 (Rollback): restore POS objects to restaurant-path state
-- Generated from pre-Migrate staging definitions (no substitutions applied)
-- MANUAL APPLY ONLY. Do not run via supabase db push.
-- ============================================================================

BEGIN;

-- ===========================================================================
-- Section A: Functions (original definitions)
-- ===========================================================================

-- Function: add_items_to_order
...
-- Function: admin_create_menu_category
...
-- Function: admin_create_menu_item
...
-- Function: admin_deactivate_restaurant
...
-- Function: admin_delete_menu_category
...
-- Function: admin_delete_menu_item
...
-- Function: admin_update_menu_category
...
-- Function: admin_update_menu_item
...
-- Function: admin_update_restaurant_settings
...
-- Function: admin_update_staff_account
...
-- Function: apply_inventory_physical_count_line
...
-- Function: complete_onboarding_account_setup
...
-- Function: confirm_delivery_settlement_received
...
-- Function: deactivate_qc_template
...
-- Function: edit_order_item_quantity
...
-- Function: get_admin_mutation_audit_trace
...
-- Function: get_admin_today_summary
...
-- Function: get_attendance_log_view
...
-- Function: get_attendance_staff_directory
...
-- Function: get_cashier_today_summary
...
-- Function: get_inventory_ingredient_catalog
...
-- Function: get_inventory_physical_count_sheet
...
-- Function: get_inventory_recipe_catalog
...
-- Function: get_inventory_transaction_visibility
...
-- Function: get_qc_superadmin_summary
...
-- Function: record_attendance_event
...
-- Function: require_admin_actor_for_restaurant
...
-- Function: update_qc_followup_status
...
-- Function: upsert_inventory_recipe_line
...
-- ===========================================================================
-- Section B: Views (original definitions)
-- ===========================================================================

-- View: public_menu_items
...
-- View: public_restaurant_profiles
...
CREATE OR REPLACE VIEW public.v_brand_kpi AS
 SELECT b.id AS brand_id,
   ...
   FROM brands b
     LEFT JOIN restaurants r ON r.brand_id = b.id AND r.store_type = 'direct'::text
     LEFT JOIN users u ON u.restaurant_id = r.id
  GROUP BY b.id, b.code, b.name
;

-- View: v_daily_revenue_by_channel
...
-- View: v_external_store_overview
...
-- View: v_store_attendance_summary
...
-- ===========================================================================
-- Section C: Policies (original definitions)
-- ===========================================================================

-- Policy: attendance_logs_policy ON attendance_logs
DROP POLICY IF EXISTS attendance_logs_policy ON public.attendance_logs;
CREATE POLICY attendance_logs_policy
...
-- Policy: settlement_items_insert ON delivery_settlement_items
...
CREATE POLICY settlement_items_insert
  ...
  WITH CHECK ((EXISTS ( SELECT 1
   FROM delivery_settlements ds
  WHERE ((ds.id = delivery_settlement_items.settlement_id) AND (ds.restaurant_id = get_user_restaurant_id())))));

-- Policy: settlement_items_read ON delivery_settlement_items
DROP POLICY IF EXISTS settlement_items_read ON public.delivery_settlement_items;
CREATE POLICY settlement_items_read
  ON public.delivery_settlement_items
  AS PERMISSIVE
  FOR SELECT
  USING ((EXISTS ( SELECT 1
   FROM delivery_settlements ds
  WHERE ((ds.id = delivery_settlement_items.settlement_id) AND (is_super_admin() OR (ds.restaurant_id = get_user_restaurant_id()))))));

-- Policy: delivery_settlements_confirm ON delivery_settlements
DROP POLICY IF EXISTS delivery_settlements_confirm ON public.delivery_settlements;
CREATE POLICY delivery_settlements_confirm
  ON public.delivery_settlements
  AS PERMISSIVE
  FOR UPDATE
  USING (((restaurant_id = get_user_restaurant_id()) AND has_any_role(ARRAY['admin'::text, 'super_admin'::text])))
  WITH CHECK ((restaurant_id = get_user_restaurant_id()));

-- Policy: delivery_settlements_read ON delivery_settlements
DROP POLICY IF EXISTS delivery_settlements_read ON public.delivery_settlements;
CREATE POLICY delivery_settlements_read
  ON public.delivery_settlements
  AS PERMISSIVE
  FOR SELECT
  USING ((is_super_admin() OR (restaurant_id = get_user_restaurant_id())));

-- Policy: external_sales_read ON external_sales
DROP POLICY IF EXISTS external_sales_read ON public.external_sales;
CREATE POLICY external_sales_read
  ON public.external_sales
  AS PERMISSIVE
  FOR SELECT
  USING ((is_super_admin() OR (restaurant_id = get_user_restaurant_id())));

-- Policy: inventory_items_policy ON inventory_items
DROP POLICY IF EXISTS inventory_items_policy ON public.inventory_items;
CREATE POLICY inventory_items_policy
...
-- Policy: restaurant_isolation ON inventory_physical_counts
...
CREATE POLICY restaurant_isolation
  ...
  AS PERMISSIVE
  FOR ALL
  USING (((restaurant_id = get_user_restaurant_id()) OR has_any_role(ARRAY['super_admin'::text])))
  WITH CHECK (((restaurant_id = get_user_restaurant_id()) OR has_any_role(ARRAY['super_admin'::text])));

-- Policy: restaurant_isolation ON inventory_transactions
...
-- Policy: menu_categories_select_policy ON menu_categories
...
-- Policy: menu_items_select_policy ON menu_items
...
-- Policy: restaurant_isolation ON menu_recipes
...
-- Policy: office_payroll_reviews_pos_update ON office_payroll_reviews
...
CREATE POLICY office_payroll_reviews_pos_update
  ON public.office_payroll_reviews
  AS PERMISSIVE
  FOR UPDATE
  TO authenticated
  USING ((has_any_role(ARRAY['admin'::text, 'super_admin'::text]) AND (is_super_admin() OR (restaurant_id = get_user_restaurant_id()))))
  WITH CHECK ((has_any_role(ARRAY['admin'::text, 'super_admin'::text]) AND (is_super_admin() OR (restaurant_id = get_user_restaurant_id()))));

-- Policy: office_payroll_reviews_scoped_select ON office_payroll_reviews
...
-- Policy: order_items_policy ON order_items
...
-- Policy: orders_policy ON orders
...
-- Policy: payments_policy ON payments
...
-- Policy: restaurant_isolation ON payroll_records
...
-- Policy: restaurant_isolation ON qc_checks
...
-- Policy: qc_followups_restaurant_isolation ON qc_followups
...
-- Policy: qc_templates_delete ON qc_templates
DROP POLICY IF EXISTS qc_templates_delete ON public.qc_templates;
CREATE POLICY qc_templates_delete
  ON public.qc_templates
  AS PERMISSIVE
  FOR DELETE
  USING ((has_any_role(ARRAY['super_admin'::text]) OR (has_any_role(ARRAY['admin'::text]) AND (is_global = false) AND (restaurant_id = get_user_restaurant_id()))));

-- Policy: qc_templates_insert ON qc_templates
DROP POLICY IF EXISTS qc_templates_insert ON public.qc_templates;
CREATE POLICY qc_templates_insert
  ON public.qc_templates
  AS PERMISSIVE
  FOR INSERT
  WITH CHECK ((has_any_role(ARRAY['super_admin'::text]) OR (has_any_role(ARRAY['admin'::text]) AND (is_global = false) AND (restaurant_id = get_user_restaurant_id()))));

-- Policy: qc_templates_select ON qc_templates
DROP POLICY IF EXISTS qc_templates_select ON public.qc_templates;
CREATE POLICY qc_templates_select
  ON public.qc_templates
  AS PERMISSIVE
  FOR SELECT
  USING (((is_global = true) OR (restaurant_id = get_user_restaurant_id()) OR has_any_role(ARRAY['super_admin'::text])));

-- Policy: qc_templates_update ON qc_templates
...
-- Policy: admin_only ON restaurant_settings
...
CREATE POLICY admin_only
  ...
  FOR ALL
  USING (((restaurant_id = get_user_restaurant_id()) AND has_any_role(ARRAY['admin'::text, 'super_admin'::text])))
  WITH CHECK (((restaurant_id = get_user_restaurant_id()) AND has_any_role(ARRAY['admin'::text, 'super_admin'::text])));

-- Policy: restaurants_select_policy ON restaurants
...
-- Policy: restaurant_isolation ON staff_wage_configs
DROP POLICY IF EXISTS restaurant_isolation ON public.staff_wage_configs;
CREATE POLICY restaurant_isolation
  ON public.staff_wage_configs
  AS PERMISSIVE
  FOR ALL
  USING (((restaurant_id = get_user_restaurant_id()) OR has_any_role(ARRAY['super_admin'::text])));

-- Policy: tables_select_policy ON tables
...
-- Policy: users_select_policy ON users
