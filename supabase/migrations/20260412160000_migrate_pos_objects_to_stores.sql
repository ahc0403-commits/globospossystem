-- ============================================================================
-- Phase 2 Step 2 (Migrate v2): rewrite POS objects to store-path compatibility
-- Generated from live staging definitions via pg_get_functiondef/pg_get_viewdef
-- Timestamp: 20260412160000
-- ============================================================================

-- NOTE: \brestaurant_id\b -> store_id substitution is NOT applied in this file.
-- Physical columns in all base tables are still named restaurant_id.
-- Applying store_id substitution would cause 'column does not exist' semantic errors.
-- Physical column rename is deferred to the Contract phase.
--
-- Applied substitutions:
--   \brestaurants\b         -> stores
--   \brestaurant_settings\b  -> store_settings
--   \bget_user_restaurant_id\b -> get_user_store_id

BEGIN;

-- ===========================================================================
-- Section A: Functions (56 total, extracted via pg_get_functiondef)
-- ===========================================================================
...
-- ===========================================================================
-- Section B: Views (11 total, extracted via pg_get_viewdef)
-- Using CREATE OR REPLACE VIEW to avoid CASCADE drop of Expand alias views
-- (public_store_profiles depends on public_restaurant_profiles)
-- ===========================================================================
...
CREATE OR REPLACE VIEW public.public_menu_items AS
 SELECT mi.id AS external_menu_item_id,
    ...
    r.operation_mode
   FROM menu_items mi
     JOIN stores r ON r.id = mi.restaurant_id
...
CREATE OR REPLACE VIEW public.v_brand_kpi AS
 SELECT b.id AS brand_id,
    ...
    ( SELECT COALESCE(sum(p.amount), 0::numeric) AS "coalesce"
           FROM payments p
             JOIN stores r2 ON r2.id = p.restaurant_id
   ...
   FROM brands b
     LEFT JOIN stores r ON r.brand_id = b.id AND r.store_type = 'direct'::text
...
CREATE OR REPLACE VIEW public.v_external_store_sales AS
 SELECT r.id AS store_id,
   ...
   FROM payments p
     JOIN stores r ON r.id = p.restaurant_id
     LEFT JOIN brands b ON b.id = r.brand_id
  WHERE r.store_type = 'external'::text
...
CREATE OR REPLACE VIEW public.v_inventory_status AS
 SELECT ii.id AS item_id,
   ...
   FROM inventory_items ii
     JOIN stores r ON r.id = ii.restaurant_id
...
CREATE OR REPLACE VIEW public.v_quality_monitoring AS
 SELECT qc.id AS check_id,
   ...
   FROM qc_checks qc
     ...
     JOIN stores r ON r.id = qc.restaurant_id
  WHERE r.store_type = 'direct'::text
;
...
CREATE OR REPLACE VIEW public.v_store_attendance_summary AS
 SELECT al.restaurant_id AS store_id,
   ...
   FROM attendance_logs al
     JOIN stores r ON r.id = al.restaurant_id
...
-- ===========================================================================
-- Section C: Policies (29 total)
-- Only get_user_restaurant_id -> get_user_store_id is applied.
-- restaurant_id column references preserved (physical column not yet renamed).
-- ===========================================================================
