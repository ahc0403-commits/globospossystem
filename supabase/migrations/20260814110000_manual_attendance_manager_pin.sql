...
DROP FUNCTION IF EXISTS public.admin_record_employee_attendance(
...
CREATE FUNCTION public.admin_record_employee_attendance(
  p_store_id uuid,
  p_employee_id uuid,
  p_type text,
...
BEGIN
  ...
  SELECT al.type INTO v_previous_type
  FROM public.attendance_logs al
  WHERE al.restaurant_id = p_store_id
    AND al.employee_id = p_employee_id
    AND al.logged_at < p_logged_at
  ORDER BY al.logged_at DESC, al.created_at DESC, al.id DESC
  LIMIT 1;

  SELECT al.type INTO v_next_type
  FROM public.attendance_logs al
  WHERE al.restaurant_id = p_store_id
    AND al.employee_id = p_employee_id
    AND al.logged_at > p_logged_at
  ORDER BY al.logged_at ASC, al.created_at ASC, al.id ASC
  LIMIT 1;

  IF p_type = 'clock_in' THEN
    IF v_previous_type = 'clock_in' OR v_next_type = 'clock_in' THEN
      RAISE EXCEPTION 'ATTENDANCE_MANUAL_SEQUENCE_INVALID';
    END IF;
  ELSE
    IF v_previous_type IS DISTINCT FROM 'clock_in' OR v_next_type = 'clock_out' THEN
      RAISE EXCEPTION 'ATTENDANCE_MANUAL_SEQUENCE_INVALID';
    END IF;
  END IF;
...
COMMENT ON FUNCTION public.admin_record_employee_attendance(
  uuid, uuid, text, timestamptz, text, text
) IS 'Allows scoped managers to backfill one past attendance event after mandatory manager PIN verification.';
