-- Expense submitter identity bridge for email-safe recipient resolution.
--
-- Grounded blocker before this migration:
-- accounting.expenses.submitted_by is TEXT only, so expense_approved and
-- expense_returned cannot resolve recipient_email deterministically.
--
-- Bounded fix:
-- 1. Add canonical office identity column on accounting.expenses.
-- 2. Populate it forward from DB-owned auth context when possible.
-- 3. Preserve legacy submitted_by TEXT for existing read models.
-- 4. Extend audit-log email enqueue to use submitted_by_auth_id for
--    expense_approved and expense_returned only.
--
-- Historical backfill:
-- No automatic backfill is performed. Existing submitted_by TEXT values are not
-- trustworthy enough to map to office_user_profiles(auth_id) without guesswork.
...
create or replace function accounting.sync_expense_submitter_identity()
returns trigger
language plpgsql
security definer
...
begin
  if new.submitted_by_auth_id is null and auth.uid() is not null then
    select *
    into v_profile
    ...
    if found then
      new.submitted_by_auth_id := v_profile.auth_id;

      if nullif(trim(coalesce(new.submitted_by, '')), '') is null then
        new.submitted_by := v_profile.display_name;
      end if;
    end if;
  elsif new.submitted_by_auth_id is not null
        and nullif(trim(coalesce(new.submitted_by, '')), '') is null then
    select *
    into v_profile
    from public.office_user_profiles oup
    where oup.auth_id = new.submitted_by_auth_id
    limit 1;
...
$$;
drop trigger if exists trg_sync_expense_submitter_identity on accounting.expenses;
create trigger trg_sync_expense_submitter_identity
before insert or update of submitted_by, submitted_by_auth_id
on accounting.expenses
for each row
execute function accounting.sync_expense_submitter_identity();
