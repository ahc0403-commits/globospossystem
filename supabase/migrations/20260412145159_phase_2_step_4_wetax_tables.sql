...
-- 4. partner_credentials
CREATE TABLE public.partner_credentials (
  id               uuid        PRIMARY KEY DEFAULT gen_random_uuid(),
  data_source      text        NOT NULL UNIQUE DEFAULT 'VNPT_EPAY',
  auth_mode        text        NOT NULL DEFAULT 'password_jwt' CHECK (auth_mode IN ('password_jwt', 'api_key')),
  user_id          text        NOT NULL,
  password_value   bytea       NOT NULL,
  password_format  text        NOT NULL DEFAULT 'plaintext' CHECK (password_format IN ('plaintext', 'aes256_ciphertext')),
...
INSERT INTO public.system_config (key, value, description) VALUES
  ('wetax_polling_enabled',                 'false',       'WT06 polling worker gate. false=disabled (default; apitest WT06 broken per Phase 2 Step 1 audit). Flip to true after vendor fix confirmed (scope Appendix B).'),
  ('wetax_dispatch_enabled',                'true',        'sendOrderInfo dispatch gate. false=emergency stop for vendor outages. Dispatcher skips all pending jobs when false.'),
  ('wetax_request_einvoice_max_retries',    '5',           'Max requestEinvoiceInfo retry attempts before job enters failed_terminal. Covers apitest POS ID not found bug (adaptation point 3).'),
  ('wetax_request_einvoice_backoff_seconds','0,3,10,30,60','Comma-separated retry intervals (seconds) for requestEinvoiceInfo backoff. Index matches retry_count: 0=immediate,1=3s,2=10s,3=30s,4=60s.');
...
CREATE TABLE public.einvoice_jobs (
  ...
  status                         text        NOT NULL DEFAULT 'pending'
                                             CHECK (status IN ('pending','dispatched','dispatched_polling_disabled','reported','issued_by_portal','failed_terminal','stale')),
...
-- 11. partner_credential_access_log
