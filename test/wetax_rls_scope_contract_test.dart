...
void main() {
  test('wetax read policies use accessible store scope without tax-axis shortcut', () {
    ...
    expect(sql, contains('CREATE OR REPLACE FUNCTION public.can_access_einvoice_job('));
    ...
    expect(sql, contains('public.can_access_einvoice_job(einvoice_events.job_id)'));
  ...
  test('direct Flutter reads stay on einvoice_jobs and rely on store-final RLS', () {
    final einvoiceTab = readRepoFile('lib/features/admin/tabs/einvoice_tab.dart');
    final reportProvider = readRepoFile('lib/features/report/report_provider.dart');
    final paymentService = readRepoFile(
      'lib/core/services/payment_service.dart',
    );

    expect(einvoiceTab, contains(".from('einvoice_jobs')"));
    expect(reportProvider, contains(".from('einvoice_jobs')"));
    expect(paymentService, contains(".from('einvoice_jobs')"));
  });

  test('sql smoke test covers sibling-store denial and multi-store allowance', () {
    ...
    expect(
      sql,
      contains(
        'brand_admin should read 2 accessible-store einvoice_jobs',
      ),
    );
    expect(
      sql,
