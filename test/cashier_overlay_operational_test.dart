...
void main() {
  ...
  for (final itemId in ['cashier-item-pho', 'cashier-item-coffee']) {
    testWidgets(
      'paperless menu cancellation ignores service checks for $itemId',
      (tester) async {
        ...
        await _selectOrder(tester);
        final cancel = find.byKey(Key('cashier_cancel_order_item_$itemId'));
        await tester.ensureVisible(cancel);
        expect(tester.widget<OutlinedButton>(cancel).onPressed, isNotNull);
        expect(find.text('Hủy món chưa phục vụ'), findsNothing);
