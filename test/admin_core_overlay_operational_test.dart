...
class _TablesNotifier extends TablesNotifier {
  ...
  Future<Map<String, dynamic>?> generateTableQr(String tableId) async {
    qrCalls += 1;
    return const {'token': 'operational-table-token'};
  }
}
    ...
    state = StaffState(
      staff: [
        StaffMember(
          ...
          authId: 'cc8b83c5-92c5-478e-a706-856cb4acc0b4',
          fullName: 'Nguyễn Minh Anh',
          role: 'waiter',
          isActive: true,
          createdAt: DateTime(2026, 7, 1),
          email: 'minh.anh@globos.test',
...
void main() {
  ...
  testWidgets('all four table dialog entrypoints execute real workflows', (
    tester,
  ) async {
  ...
  testWidgets('all four staff sheet and dialog entrypoints execute', (
    ...
    await tester.tap(find.byKey(const Key('admin_staff_permission_action')));
    await tester.pumpAndSettle();
    const permissionDialog = Key('admin_staff_permission_dialog');
    expect(find.byKey(permissionDialog), findsOneWidget);
    _expectDialogButtonsAreTouchSized(tester, permissionDialog);
    await tester.tap(
      find
          .descendant(
            of: find.byKey(permissionDialog),
            matching: find.byType(CheckboxListTile),
          )
          .at(1),
    );
    await tester.tap(_dialogAction(permissionDialog, FilledButton));
    await tester.pumpAndSettle();
    expect(staffNotifier.permissionCalls, 1);
    ...
    await tester.enterText(fields.at(1), 'gia.huy@globos.test');
    await tester.enterText(fields.at(2), 'secure123');
    await tester.tap(addButton);
    await tester.pumpAndSettle();
    expect(staffNotifier.createCalls, 1);
    const credentialsDialog = Key('staff_created_credentials_dialog');
    expect(find.byKey(credentialsDialog), findsOneWidget);
    expect(find.text('gia.huy@globos.test'), findsOneWidget);
    expect(find.text('secure123'), findsOneWidget);
    await tester.tap(_dialogAction(credentialsDialog, FilledButton));
    await tester.pumpAndSettle();
    expect(tester.takeException(), isNull);
  });
