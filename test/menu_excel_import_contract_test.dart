...
void main() {
  ...
  test(
    'replacement import retains matching photos and removes absent rows',
    () {
      final sql = File(
      ...
      expect(sql, contains('DELETE FROM public.menu_items mi'));
      expect(sql, contains('DELETE FROM public.menu_categories c'));
      ...
      expect(
        deployment,
        contains('20260805140000_admin_menu_excel_replace.sql'),
      );
      expect(deployment, contains('preflight_admin_menu_excel_replace.sql'));
      expect(deployment, contains('verify_admin_menu_excel_replace.sql'));
    },
  );
}
