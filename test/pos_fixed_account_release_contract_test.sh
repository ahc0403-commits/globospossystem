...
grep -Fq 'supabase functions deploy complete-initial-password-change' "$DEPLOY"
grep -Fq 'grep -Eq "\"projectName\"[[:space:]]*:[[:space:]]*' "$DEPLOY"
grep -Fq 'grep -Eq "\"projectId\"[[:space:]]*:[[:space:]]*' "$DEPLOY"
grep -Fq 'grep -Eq "\"orgId\"[[:space:]]*:[[:space:]]*' "$DEPLOY"
grep -Fq 'verify_remote_allowed_origin' "$DEPLOY"
grep -Fq 'access-control-allow-origin' "$DEPLOY"
grep -Fq 'sub(/^[^:]*:[[:space:]]*/, "", value)' "$DEPLOY"
